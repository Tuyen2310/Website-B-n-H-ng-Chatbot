...
const nextConfig: NextConfig = {
  ...
  async rewrites() {
    return [
      {
        source: '/api/:path*',
        destination: `${process.env.NEXT_PUBLIC_API_URL ? (process.env.NEXT_PUBLIC_API_URL.endsWith('/api') ? process.env.NEXT_PUBLIC_API_URL.slice(0, -4) : process.env.NEXT_PUBLIC_API_URL) : 'http://127.0.0.1:3001'}/api/:path*`,
      },
    ];
  },
};
