...
async function main() {
  ...
  for (const [catName, products] of Object.entries(realisticData)) {
    ...
    for (const [index, p] of products.entries()) {
      // Dùng loremflickr với từ khóa để có ảnh đúng chủ đề
      ...
      await (prisma.product.create as any)({
        data: {
          ...
          soldCount: Math.floor(Math.random() * 500) + 10, // Thêm soldCount ngẫu nhiên
  ...
  // Khởi tạo vài người dùng và wishlist để test chức năng
