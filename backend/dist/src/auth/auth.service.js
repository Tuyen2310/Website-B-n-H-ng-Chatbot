...
let AuthService = class AuthService {
    ...
    async register(registerDto) {
        ...
        if (existing) {
            throw new common_1.ConflictException('Email này đã được sử dụng. Vui lòng chọn email khác.');
        }
    ...
    async validateOAuthLogin(profile) {
        ...
        if (user) {
            if (!user.googleId) {
                user = await this.usersService.update(user.id, { googleId, avatar: picture });
            }
        }
        else {
